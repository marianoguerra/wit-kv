...
//! - `val_lower`: Direct wasmtime Val lowering (requires `wasm` feature)
//! - `val_lift`: Direct wasmtime Val lifting (requires `wasm` feature)
//!
//! # Convenience Types
//!
...
impl<'a> CanonicalAbi<'a> {
    ...
    ///
    /// ```ignore
    /// use wit_kv::abi::{CanonicalAbi, EncodedValue};
    ///
    /// let abi = CanonicalAbi::new(&resolve);
