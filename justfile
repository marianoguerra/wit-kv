...
build-all: build build-client build-examples build-playground

# Create a distribution with server, client UI, playground, and example config
dist: build-all
    #!/usr/bin/env bash
    set -e
    mkdir -p dist
    ...
    # Create a sample config with static_path configured (playground by default)
    ...
    echo "  - wit-kv-server.toml (config with playground enabled)"
    echo ""
